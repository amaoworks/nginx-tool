...
| 编号 | 项 | 状态 | 说明 |
|------|----|------|------|
| P1.1 | 站点列表表格排序 | **已完成**（v1.2.4） | 键位落地为 `o` 切字段、`p` 升降序（`s` 保留给启停） |
| P1.2 | 日志搜索高亮与匹配计数 | **已完成**（v1.2.4） | 高亮、`n`/`N` 跳转、匹配计数已实现 |
| P1.3 | 配置编辑 Diff 预览 | 待做 | 配置健康扫描/修复已单独落地，与 diff 预览互补 |
| P2.1 | 彩色站点类型标识 | 待做 | SSL 列已有色标，类型列仍可增强 |
| P2.2 | 自适应表格列宽 | 部分完成 | 有 `Min` 与宽度估算，窄终端仍偏硬编码 |
| P2.3 | 操作进度指示 | 待做 | — |
| P3.1 | 仪表盘并行采集 | **已完成** | `domain/dashboard.rs` 使用 `tokio::join!` + 单项 timeout |
| P3.2 | 站点列表文件监听 | 待做 | — |
...
**目标（及落地）：**
- 支持按 状态/名称/类型/SSL 排序
- 按键：**`o` 切换排序字段，`p` 切换升序/降序**（因 `s` 已用于启停，未采用初稿的 `s`/`o`）
- 表头与页内状态显示当前排序规则（如 `排序: 状态↓`）
...
- `src/ui/mod.rs`：底部状态栏增加排序提示 `[o] 排序`（配合 `p` 升降序）

**测试点：**
- 排序后光标保持在同一站点（而非同一索引）— 已有单测
- 刷新列表后排序规则保持
...
**状态：部分完成**（有 `Constraint::Min` 与 `saturating_sub` 估算；窄终端仍偏硬编码，待继续打磨）

**现状问题：**
- `sites.rs` 仍用 `area.width.saturating_sub(54).max(50)` 一类估算
- 窗口缩放时列宽表现仍可进一步比例化
...
**原问题：**
- `domain/dashboard.rs` 中各指标串行采集，总耗时 = 所有探测时间之和

**目标（及落地）：**
- 用 `tokio::join!` 并行采集，总耗时 ≈ 最慢的探测时间
- 单项 3s timeout，失败不影响其他项
...
**已完成**
- P1.1 站点列表排序（v1.2.4）
- P1.2 日志搜索高亮与匹配计数（v1.2.4）
- P3.1 仪表盘并行采集

**下一优先**
1. P1.3 配置 diff 预览（含 similar crate 集成）
2. P2.3 操作进度指示（spinner + 时长）
3. P2.1 / P2.2 视觉与列宽打磨
4. P3.2 文件监听（含 notify crate + debounce）

### 质量保证

**每个功能完成后：**
1. 运行 `cargo test` / `cargo clippy -- -D warnings` 确保无回归（亦由 `.github/workflows/ci.yml` 覆盖）
2. 手动测试核心流程（新建站点 → 编辑 → 启用 → 删除）
3. 更新 `CHANGELOG.md` 记录变更
4. 更新 `doc/design.md` 补充新增的交互规范（需许可）
...
# 以下仍为待做项预留，尚未引入：
[dependencies]
similar = "2.6"   # P1.3 diff 预览（待做）
notify = "6.1"    # P3.2 文件监听（待做）
```
...
- [x] `CHANGELOG.md`：回溯整理 v1.1.1 ~ v1.2.4，并记录 Unreleased
- [ ] `README.md`：补充排序 / 日志搜索等用户可见键位
...
| notify 跨平台兼容性 | macOS/Windows 行为差异 | 文档说明 Linux 优先；CI 以 Linux 为主 |
| 并行采集导致超时控制复杂 | 某个探测卡住影响整体 | 已落地：每个探测独立 timeout + `tokio::join!` |
...
- 国际化（i18n）
- 插件系统（自定义站点模板）
- `state.rs` 拆分（可维护性，与功能正交）

---

**文档版本**：v1.1  
**创建日期**：2026-06-03  
**最近更新**：2026-07-10（同步已完成项与真实键位）  
**适用版本**：tui-next >= v1.2.4
